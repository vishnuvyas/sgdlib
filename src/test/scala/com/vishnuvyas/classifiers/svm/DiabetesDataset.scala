...
class DiabetesDataset extends Dataset[Array[Double],java.lang.Boolean] {
  ...
  def initialize() = {
    val rawdataset = Source.fromInputStream(dataFileStream).getLines().map { line =>
      val a = line.trim().split(",").map(_.toDouble)
    ...
    val scaleInfo = rawdataset.foldLeft(Map[Int,(Double,Double,Int)]()) { (m,d) =>
      d._1.zipWithIndex.foldLeft(m) { (mi,f) =>
        val default = mi.getOrElse(f._2,(0.0,0.0,0))
    ...
    val rescaledDataset = rawdataset.map { d =>
      val rescaledVector = d._1.zipWithIndex.map { case (v,f) =>
        (v - rescaleInfo(f)._1)/rescaleInfo(f)._2
      }
      (rescaledVector -> d._2)
    }
  ...
  override def getPoint(i: Int): Array[Double] = dataset(i)._1
  ...
  override def sample(fraction: Double): Dataset[Array[Double], java.lang.Boolean] = new Dataset[Array[Double],java.lang.Boolean] {
    val sampleDataset = dataset.filter(_ => Random.nextDouble() < fraction)

    override def count(): Int = sampleDataset.size
    override def getPoint(i: Int) : Array[Double] = sampleDataset(i)._1
    override def getLabel(i : Int) : java.lang.Boolean = sampleDataset(i)._2
    override def getLabels : util.Set[java.lang.Boolean] = Set(java.lang.Boolean.TRUE,java.lang.Boolean.FALSE)
    override def dim(): Int = 8

    override def sample(fraction: Double): Dataset[Array[Double], java.lang.Boolean] = ???
  }
  ...
  def confusionMatrix(c:Classifier[Array[Double],java.lang.Boolean]) = {
